...
public class CarController :MonoBehaviour
{
    ...
    #region DriftScore logic
    public int GetScore { get { return Score; } }
    public int GetCurrentScore { get { return CurrentScore; } }
    public int GetScoreMultiplier { get { return ScoreMultiplier; } }
	
	void UpdateDriftScoreLogic()
	{
        int driftVector = Mathf.Abs((int)Vector3.Dot(COM.right, _RB.velocity));
        ...
        if (driftVector >= 8)
        {
            ...
            // Изменение мультипликатора, в зависимости от времени дрифта
            switch ((int)(Time.time - StartDriftTime) / 5)
            {
                case 0:
                    ScoreMultiplier = 1;
                    break;
            ...
            CurrentScore += (int)(Time.deltaTime * ScoreMultiplier * 250);
        }
		else // Вышли из дрифта
		{
			if(EndDriftTime == 0)
                EndDriftTime = Time.time;

			if (Time.time - EndDriftTime > 2)
            {
				Score += CurrentScore;
                CurrentScore = 0;
				ScoreMultiplier = 1;
